...
static int8_t create_task(tipo_e tipo) {
  ...
  int8_t task_id = sched_add_task(gdt_id << 3); //shifteo 3, porq estoy pasando un selector, los ultimos 3 bits son atributos
  tss_tasks[task_id] = tss_create_user_task(task_code_start[tipo]); //creamos el TSS
  gdt[gdt_id] = tss_gdt_entry_for_task(&tss_tasks[task_id]); //define el descriptor de segmento del TSS en la gdt
  return task_id;
}
...
void tasks_screen_draw(void) {
  ...
  print("Tarea 1 - B", 43,  0, C_FG_WHITE | C_BG_BLUE);
  print("Tarea 2 - A",  3, 25, C_FG_WHITE | C_BG_RED);
